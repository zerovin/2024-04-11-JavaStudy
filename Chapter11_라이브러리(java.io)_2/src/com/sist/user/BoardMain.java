...
public class BoardMain extends JFrame implements ActionListener, MouseListener{
	...
	BoardSystem bs=new BoardSystem(); //파일에 List 저장되어있음
	public BoardMain() {
		setLayout(card);
		add("LIST", bList);
		add("INSERT",bInsert);
		...
		setSize(640,550);
		setVisible(true);
		setDefaultCloseOperation(EXIT_ON_CLOSE); //닫으면 자동 메모리 해제
		
		//등록 => actionPerformed 호출
		bList.inBtn.addActionListener(this); //새글
		bList.prevBtn.addActionListener(this); //prev
		bList.nextBtn.addActionListener(this); //next
		bList.table.addMouseListener(this); //상세
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(bList.inBtn==e.getSource()) { //새글 버튼 클릭
		...
		}else if(bInsert.b2==e.getSource()) { //취소 버튼 클릭
			...
			String pwd=String.valueOf(bInsert.pwdPf.getPassword()); //비밀번호 리턴
			...
			card.show(getContentPane(), "LIST");
			listPrint();
		}else if(bDetail.b1==e.getSource()) { //상세 - 수정 버튼 클릭
			//게시물 번호 읽기
		...
		}else if(bDetail.b2==e.getSource()) { //상세 - 삭제 버튼 클릭
			//게시물 번호 읽기
			String no=bDetail.no.getText();
		...
		}else if(bDetail.b3==e.getSource()) { //상세 - 목록 버튼 클릭
			card.show(getContentPane(), "LIST");
			listPrint();
		}else if(bUpdate.b1==e.getSource()) { //수정 - 글쓰기 버튼 클릭
			//입력값 읽기
			...
			String pwd=String.valueOf(bUpdate.pwdPf.getPassword()); //비밀번호 리턴
			if(pwd.length()<1) { //입력이 안된 경우
			...
			card.show(getContentPane(), "LIST");
			listPrint();
		}else if(bUpdate.b2==e.getSource()) { //수정 - 취소 버튼 클릭
