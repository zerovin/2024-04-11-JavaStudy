...
public class 멤버변수_초기화 {
	...
	Movie[] movies=new Movie[1938]; //선언만 하는 영역 null값
	//멤버변수 => 프로그램 종료시까지 유지
	//초기화 => 시작
	//파일 읽기 => 구현
	{
		try {
			...
			while((i=fr.read())!=-1) { //-1 EOF
				sb.append((char)i);  //저장 속도가 +=보다 Buffer가 훨씬 빠름
				//m+=(char)i;
			}
			fr.close();
			...
			int j=0;
			for(String mm:movie) {
				movies[j]=new Movie(); //클래스 초기화 => 저장공간확보 
	...
	public static void main(String[] args) {
		...
		System.out.println("출연한 영화 총 "+count+"건 입니다");
		...
		System.out.println("영화 총 "+count+"건 입니다");
		
		System.out.println("-------- 15세 관람가 출력 ---------");
		count=0;
		for(Movie movie:m.movies) {
			if(movie.grade.contains("15")) {
				System.out.println(movie.title+" - "+movie.actor);
				count++;
			}
		}
		System.out.println("15세 관람가 영화 총 "+count+"건 입니다");
		...
		System.out.println("총 "+count+"건 입니다");
		...
		*/
		...
		System.out.print("1~1938 사이의 정수 입력 >> ");
		int no=scan.nextInt();
		...
		System.out.println("감동:"+mm.director);
	}
