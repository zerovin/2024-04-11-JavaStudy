 ...
 *	String 일반 문자열 데이터형 / 클래스형 => 문자열을 제어하는 기능(메소드)
 *
 *	String 사용법
 *	String name="ASSS"; // 제한이 없다 char[]을 클래스화
 *	
 *	Wrapper - 데이터형을 제어하는 클래스
 *	  int => Integer
 *	  long => Long
 *	  byte => Byte
 *	  double => Double
 ...
 *	배열
 *	String[] 배열명={} => 값 고정
 *	String 배열명=new String[10]; => 값 초기화, 변경
 */
import java.util.*;
public class 문자열배열_1 {
	...
	public static void main(String[] args) {
		...
		int count=0;
		for(String t:title) {
			if(t.contains(fd)) { // 제목에 검색어가 포함되어 있는지 여부 true/false
		...
		// 배열이 여러개인 경우 => 인덱스 번호를 맞춰 인덱스로 제어
		// 상승 / 하락 / 유지
		
		for(int i=0;i<title.length;i++) { // 여러개 배열 동시제어 - 인덱스 이용
			String temp="";
